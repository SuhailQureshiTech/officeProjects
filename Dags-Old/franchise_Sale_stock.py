...
dag = models.DAG('Franchise_dag', default_args=default_args,schedule_interval='@once')
...
def insert_stock_data_to_gcs():
    df = pd.read_excel(f'/home/admin2/airflow/dags/files/AZEE_DT138_06JUN.xlsx',sheet_name='StockView')
    ...
    df.columns = ['ibl_distributor_code','distributor_item_code','dated','ibl_item_code','distributor_item_description','lot_number','expiry_date','stock_qty','stock_value']
...
# curr_date = today.strftime("%d-%b-%Y")
    ...
    bucket.blob(f'staging/franchise/stock/{filename}.csv').upload_from_string(df.to_csv(index=False), 'text/csv')
...
t2 = gcs_to_bq.GoogleCloudStorageToBigQueryOperator(
    ...
    {'name': 'distributor_item_description', 'type': 'STRING', 'mode': 'NULLABLE'},
...
def insert_sale_data_to_gcs():
    df = pd.read_excel(f'/home/admin2/airflow/dags/files/AZEE_DT138_06JUN.xlsx',sheet_name='SaleFormate')
    ...
    df.columns = ['order_no','invoice_date','invoice_no','channel','ibl_distributor_code','distributor_customer_no','ibl_customer_no','customer_name','distributor_item_code','ibl_item_code','item_description','qty_sold','gross_amount','reason','discount','bonus_qty' ]
    # df['dated'] = datetime.today().strftime('%Y-%m-%d')
    df['company_code'] = '1001'
    df['transfer_date'] = datetime.today()
    company_code='1001'
...
# curr_date = today.strftime("%d-%b-%Y")
    ...
    bucket.blob(f'staging/franchise/sales/{filename}.csv').upload_from_string(df.to_csv(index=False), 'text/csv')
