...
# # ###############################################BookingExecution########################################
  ...
  -- WHERE billing_Date BETWEEN  '2022-04-21' and '2022-04-26'
  ...
  FROM `data-light-house-prod.EDW.IBL_SALES` PSD
  WHERE 1 = 1
  AND PSD.billing_date BETWEEN  (current_date()-10) and (current_date()-1) --{vStartDate} and {vEndDate}
  ...
  #   ############################################## FULL LOAD BookingExecution########################################


    #   Full_Load_Booking_Execution_Data_to_gcs = PythonOperator(
    #         task_id="Full_Load_Booking_Execution_Data_to_gcs",
    #         python_callable=Full_Load_Booking_Execution_Data_to_gcs,
    #         dag=dag,
    # )

#       Full_Load_Booking_Execution_data_gcs_to_bq = GCSToBigQueryOperator(
#         task_id='Full_Load_Booking_Execution_data_gcs_to_bq',
#         bucket='ibloper',
#         source_objects=f'staging/sales/BOOKING_EXECUTION/IBL_BOOKING_EXECUTION.csv',
#         destination_project_dataset_table='data-light-house-prod.EDW.IBL_BOOKING_EXECUTION',
#         schema_fields=[
#         # Define schema as per the csv placed in google cloud storage
...
#         {'name': 'booker_code', 'type': 'STRING', 'mode': 'NULLABLE'},
...
#         {'name': 'return_amount', 'type': 'numeric', 'mode': 'NULLABLE'},
#         {'name': 'cancelled', 'type': 'Numeric', 'mode': 'NULLABLE'},
#         {'name': 'gross_amount', 'type': 'numeric', 'mode': 'NULLABLE'},
#         {'name': 'data_flag', 'type': 'numeric', 'mode': 'NULLABLE'},
...
#         dag=dag
#         )
########################################## Full Load Salesflo DATA######################################################################
    # Full_Load_Saleflo_Data_to_gcs = PythonOperator(
    #         task_id="Full_Load_Saleflo_Data_to_gcs",
    #         python_callable=Full_Load_Saleflo_Data_to_gcs,
    #         dag=dag,
    # )



    # Full_Load_saleflo_data_gcs_to_bq = GCSToBigQueryOperator(
    # task_id='Full_Load_saleflo_data_gcs_to_bq',
    # bucket='ibloper',
    # source_objects=f'staging/sales/Salesflo_Orders/SALESFLO_ORDERS.csv',
    # destination_project_dataset_table='data-light-house-prod.EDW.IBL_SALESFLO_ORDERS',
    # schema_fields=[
    # # Define schema as per the csv placed in google cloud storage
    # {'name': 'distributor_name', 'type': 'STRING', 'mode': 'NULLABLE'},
    ...
    # {'name': 'delivery_date', 'type': 'date', 'mode': 'NULLABLE'},
    # {'name': 'added_on', 'type': 'date', 'mode': 'NULLABLE'},
    # {'name': 'updated_on', 'type': 'date', 'mode': 'NULLABLE'},
    #  {'name': 'filename', 'type': 'STRING', 'mode': 'NULLABLE'},
    # {'name': 'transfer_date', 'type': 'TIMESTAMP'}
    # ],
    # write_disposition='WRITE_TRUNCATE',
    ...
    # Load_sap_item_bus_line_data_gcs_to_bq = GCSToBigQueryOperator(
    # task_id='Load_sap_item_bus_line_data_gcs_to_bq',
    # bucket='ibloper',
    # source_objects=f'staging/sales/SAP_ITEM_BUS_LINE.csv',
    # destination_project_dataset_table='data-light-house-prod.EDW.SAP_ITEM_BUSINESS_LINE',
    # schema_fields=[
    # # Define schema as per the csv placed in google cloud storage
    # {'name': 'item_code', 'type': 'STRING', 'mode': 'NULLABLE'},
