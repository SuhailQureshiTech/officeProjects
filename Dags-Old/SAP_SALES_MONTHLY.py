...
if day <=3:
    ...
    vEndDate = datetime.today() - timedelta(days=day_diff)

    vSapFromDate = vStartDate.strftime("%Y%m%d")
    vSapEndDate = vEndDate.strftime("%Y%m%d")

    vSapFromDate = "'"+str(vSapFromDate)+"'"
    vSapEndDate = "'"+str(vSapEndDate)+"'"
    ...
    vStartDate = datetime.today().replace(day=1)
...
# print('sap from date :', vSapFromDate)
# print('sap end date :', vSapEndDate)
...
def insertSapSales():
    ...
    select
        ...
        where
            1 = 1
            and "FKDAT"  BETWEEN {vSapFromDate} and {vSapEndDate}
...
def insertData():
    ...
    cursor = conn2.cursor()
    delete_sql = f'''DELETE FROM "DW"."IBL_SALES" where billing_Date BETWEEN {vStartDate}  AND {vEndDate}
...
def Load_Sale_Data_to_gcs():
   ...
   # print(current_Datetime)
    df = pds.read_sql(f'''select item_category,branch_id,branch_description,billing_date,document_no,booker_id,booker_name,supplier_id,supplier_name,business_line_id,business_line_description,channel,customer_id,customer_number,customer_name,sales_order_type,inventory_item_id,item_code,item_description,unit_selling_price,sold_qty,bonus_qty,claimable_discount,unclaimable_discount,tax_recoverable,net_amount,gross_amount,total_discount,reason_code,bill_type_description,company_code,billing_type,sales_order_no,address_1,address_2,address_3,salesflo_order_no,item_no,return_reason_code
            from "DW"."IBL_SALES" where billing_date  BETWEEN {vStartDate}  AND {vEndDate}
            and company_code in ('6300','6100') ''',conn)
    df['transfer_date'] =creationDate
    # filename =f'IBL_SALES_{d1}'
    filename =f'IBL_SALES_RANGE'
...
# with DAG(
#         dag_id='SAP_SALES_MONTHLY',
#         # start date:28-03-2017
#         start_date= datetime(year=2022, month=11, day=28),
#         # run this dag at 2 hours 30 min interval from 00:00 28-03-2017
#         schedule_interval='30 15,17 * * *',
...
with DAG(
    ...
    sql=f'''
        delete from  `data-light-house-prod.EDW.IBL_SALES_BACKDATED`
        where Billing_date  BETWEEN {vStartDate}  AND {vEndDate}
        and company_code in ('6300','6100')
        ''',
    dag=dag)
    ...
    bucket='ibloper',
    source_objects=f'staging/sales/FiscalYear/2021-22/IBL_SALES_RANGE.csv',
    # source_objects=f'staging/sales/FiscalYear/2021-22/IBL_SALES_2022-05-10.csv',
    destination_project_dataset_table='data-light-house-prod.EDW.IBL_SALES_BACKDATED',
    schema_fields=[
