...
# credentialsData = service_account.Credentials.from_service_account_file(
...
vMaxDate1 = datetime.date(datetime.today()-timedelta(days=1)).strftime("%Y-%m-%d")
...
# checkFileTask = PythonOperator(
#         task_id="checkFile",
#         python_callable=fileCheck,
#         dag=sap_sale_merging
#     )

# writeFileTask = PythonOperator(
#         task_id="writeFile",
...
#         columns={'ITEM_CATEGORY': 'item_category', 'BRANCH_ID': 'branch_id', 'BRANCH_DESCRIPTION': 'branch_description', 'BILLING_DATE': 'billing_date', 'DOCUMENT_NO': 'document_no', 'BOOKER_ID': 'booker_id', 'BOOKER_NAME': 'booker_name', 'SUPPLIER_ID': 'supplier_id', 'SUPPLIER_NAME': 'supplier_name', 'BUSINESS_LINE_ID': 'business_line_id', 'BUSINESS_LINE_DESCRIPTION': 'business_line_description', 'CHANNEL': 'channel', 'CUSTOMER_ID': 'customer_id', 'CUSTOMER_NUMBER': 'customer_number', 'CUSTOMER_NAME': 'customer_name', 'SALES_ORDER_TYPE': 'sales_order_type', 'INVENTORY_ITEM_ID': 'inventory_item_id', 'ITEM_CODE': 'item_code', 'ITEM_DESCRIPTION': 'item_description', 'UNIT_SELLING_PRICE': 'unit_selling_price', 'SOLD_QTY': 'sold_qty', 'BONUS_QTY': 'bonus_qty', 'CLAIMABLE_DISCOUNT': 'claimable_discount', 'UNCLAIMABLE_DISCOUNT': 'unclaimable_discount', 'TAX_RECOVERABLE': 'tax_recoverable', 'NET_AMOUNT': 'net_amount', 'GROSS_AMOUNT': 'gross_amount', 'TOTAL_DISCOUNT': 'total_discount', 'REASON_CODE': 'reason_code', 'BILL_TYPE_DESCRIPTION': 'bill_type_description', 'COMPANY_CODE': 'company_code', 'BILLING_TYPE': 'billing_type', 'SALES_ORDER_NO': 'sales_order_no', 'ADDRESS_1': 'address_1', 'ADDRESS_2': 'address_2', 'ADDRESS_3': 'address_3', 'SALESFLO_ORDER_NO': 'salesflo_order_no', 'ITEM_NO': 'item_no', 'RETURN_REASON_CODE': 'return_reason_code'})
#     if len(data_frame) > 0:
#         print('in if block...')
...
# checkFileTask>> delDataTask>>insertSapSalesTask>>LoadSalesDataGcsTask>>Delete_From_BQ>>Load_sale_data_gcs_to_bq>>ReadDataGcsTask>>writeFileTask
