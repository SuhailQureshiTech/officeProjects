...
ibl_markitt = DAG(
    dag_id='Markitt_Sales_Merging_BQ',
...
def extract_markitt_sales_to_gcs():
    ...
    query = f''' SELECT
        ...
        , sal.Rate as rate
        , PurAvgRate as pur_avg_rate
        , Amount as amount
        , ItemDiscountPercentage as item_discount_percentage
        , ItemDiscountAmount as item_discount_amount
        , sal.DiscountAmount as discount_amount
        , NetAmount as net_amount
        , sal.isLoyalty as is_loyalty
        , sal.SupplierCode as supplier_code
        , trDateTime as tr_datetime
        , IsImported as is_imported
        , IsExcluded as is_excluded
        , DealFactor as deal_factor
        , DealValue as deal_value
        , DealOperator as deal_operator
        , OfferAmount as offer_amount
        , OfferFactor as offer_factor
        , NoOfDeals as no_of_deals
        , DealID as deal_id
        , POQuantity as po_quantity
        , ItemPurchaseRate as item_purchase_rate
        , ItemGSTPercentage as item_GST_percentage
        ...
        FROM 	dbo.INV_PointofSalesDetailTAB sal
...
deleteSalesBQRecords = BigQueryOperator(
    task_id='DeleteSalesBQRecords', bigquery_conn_id='bigquery',
    use_legacy_sql=False, sql=f'''
            truncate table  `data-light-house-prod.EDW.IBL_MARKITT_INV_POS`
        ''',
    dag=ibl_markitt
)
...
deleteSalesReturnBQRecords = BigQueryOperator(
    task_id='DeleteSalesReturnBQRecords', bigquery_conn_id='bigquery',
    use_legacy_sql=False, sql=f'''
        truncate table  `data-light-house-prod.EDW.IBL_MARKITT_INV_RET_POS`
        ''',
    dag=ibl_markitt
)
...
def extract_markitt_salesReturn_to_gcs():
    ...
    query = f''' SELECT
        ...
        ItemGSTAmount,
        NetAmountWithoutGST,
        itmcat.categorycode,category,itmcat .brandcode,brand
        from dbo.INV_SalesReturnDetailTAB ret
        left outer join dbo.vw_item_brand_categ_data as  itmcat on (itmcat.barcode=ret.BarCode)
        left outer join INV_ItemTAB iit on (iit.BarCode=ret.BarCode)
	    where  cast(BillDate as date)<={dataDate}
        '''
    ...
    client = storage.Client(project=GCS_PROJECT)
    bucket = client.get_bucket(GCS_BUCKET)
    filename =f'ibl_markitt_return_{curr_date}'
...
markitt_return_data_gcs_to_bq = GCSToBigQueryOperator(
    ...
    bucket='ibloper',
    source_objects=f'staging/markitt/ibl_markitt_return_{curr_date}.csv',
    destination_project_dataset_table='data-light-house-prod.EDW.IBL_MARKITT_INV_RET_POS',
    schema_fields=[
    # Define schema as per the csv placed in google cloud storage
        {'name': 'integration', 'type': 'STRING', 'mode': 'NULLABLE'},
