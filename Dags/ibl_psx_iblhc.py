...
def success_function(context):
    ...
    msg = "IBLHC Stock Exchange DAG has executed successfully."
    subject = f"IBLHC Stock Exchange DAG has completed"
...
def extract_psx_iblhc_data_scrap_to_postgres():

    headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/89.0.4389.82 Safari/537.36'}
    url = 'https://dps.psx.com.pk/company/IBLHL'
    ...
    reg_Stat = {stat_lbl[i]: stat_value[i] for i in range(len(stat_lbl))}
    ...
    fut_Stat = {fut_stat_lbl[i]: fut_stat_value[i] for i in range(len(fut_stat_lbl))}
    ...
    company_df.insert(0,"company_code",1200)    
    ...
    (
        ...
        FROM "DW"."IBL_PSX" where company_code = 1200
    ) AS v_table_name
...
def insert_PSX_IBLHC_data_To_GCS():
    df = pd.read_sql('select *  from "DW"."IBL_PSX" where company_code =\'1200\' order by company_code,created_at desc limit 1', conn);
    df_IBLHC = pd.DataFrame(data=df)
    ...
    filename =f'ibl_psx_iblhc_{curr_date}'

    bucket.blob(f'staging/psx/iblhc/{filename}.csv').upload_from_string(df_IBLHC.to_csv(index=False), 'text/csv')
    conn.close()             
           


ibl_psx_iblhc = DAG(
    dag_id='ibl_psx_iblhc',
    default_args=default_args,
    schedule_interval='0 04-11 * * *',    
   # dagrun_timeout=timedelta(minutes=60),
    description='ibl_psx_iblhc',
)


psx_iblhc_data_scrap_to_postgres = PythonOperator(
        task_id="psx_iblhc_data_scrap_to_postgres",
        python_callable=extract_psx_iblhc_data_scrap_to_postgres,
        dag=ibl_psx_iblhc
)


psx_iblhc_postgres_to_GCS =  PythonOperator(
    task_id="psx_iblhc_data_to_GCS",
    python_callable=insert_PSX_IBLHC_data_To_GCS,
    dag=ibl_psx_iblhc
)

psx_iblhc_data_gcs_to_bq = GCSToBigQueryOperator(    
    task_id='psx_iblhc_data_gcs_to_bq',
    bucket='ibloper',
    source_objects=f'staging/psx/iblhc/ibl_psx_iblhc_{curr_date}.csv',
...
psx_iblhc_data_scrap_to_postgres >> psx_iblhc_postgres_to_GCS >> psx_iblhc_data_gcs_to_bq
