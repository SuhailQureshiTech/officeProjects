    ...
    FROM(
        ...
        ' '  AS BATCH_NO,
         unit_selling_price AS price,
            cast(SUM(sold_qty) as int) AS SOLD_QTY,
       ...
       where 1 = 1 AND billing_date  between '2022-07-01' and '2022-07-31' 
       ---and branch_id='8032' and data_flag='SD' and  upper(item_description)  like 'NUB%15%'
               GROUP BY BR_CD,
        document_no,
        TRX_DATE1      ,
...
def genSalesTextFile(dataFrame1):
    ...
    dataFrame1.to_csv(f'''{vPath}ALL_LOC_SALE_CSV.csv''',
...
def BigQueryCustomerData():
    ...
    sqlQuery = f'''       
      select distinct 
        ...
        ,ifnull(address_1,'') ADD1
        ,concat(ifnull(address_2,''),ifnull(address_3,'')) ADD2
        , data_flag
        from `data-light-house-prod.EDW.VW_EBS_SAS_ALL_LOC_DATA_NEW` 
         where billing_date between '2022-07-01' and '2022-07-31'  and branch_id is not null           
    '''
...
def genCustomerTextFile():
    ...
    dataFrame2[var]=dataFrame1['BR_CD']+conString+dataFrame1['EBS_CUST']+conString+dataFrame1['CUSTOMER_NAME']+conString+dataFrame1['CH_CD']+conString+dataFrame1['ADD1']+conString+dataFrame1['ADD2']+conString+dataFrame1['data_flag']+conString
