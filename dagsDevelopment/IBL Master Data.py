...
deleteLocationBQRecordsTask = BigQueryOperator(
    ...
    sql=f'''truncate table `data-light-house-prod.EDW.LOCATION_DETAILS` ''',
    dag=masterDataIntg
)
...
deleteCustomerBQRecordsTask = BigQueryOperator(
    task_id='DeleteCustomerBQRecords',
    use_legacy_sql=False, sql=f''' truncate table `data-light-house-prod.EDW.IBL_CUSTOMERS_DATA`
        ''',
    dag=masterDataIntg
)
...
def generateCustomerData():
    ...
    for char in spec_chars:
        df['ADD1'] = df['ADD1'].str.replace(
            char, ' ', regex=True)
        df['ADD1'] = df['ADD1'].str.split().str.join(" ")

    for char in spec_chars:
        df['ADD2'] = df['ADD2'].str.replace(
            char, ' ', regex=True)
        df['ADD2'] = df['ADD2'].str.split().str.join(" ")

    for char in spec_chars:
        df['ADD3'] = df['ADD3'].str.replace(
            char, ' ', regex=True)
        df['ADD3'] = df['ADD3'].str.split().str.join(" ")
