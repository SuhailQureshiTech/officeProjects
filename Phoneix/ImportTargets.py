...
path = r'D:\\TEMP\\Targetsworking.csv'
df = pd.read_csv(
        path
    , usecols=[
        'BUSINESS LINE'	,'BUSINESS LINE NAME','MONTH','YEAR','TARGET'
                ]
                ,low_memory=False )
print(df)

   # Changing Type
# df["TARGET"] = df["TARGET"].astype(Number)
...
def insertData():
        ...
        for index, row in df.iterrows():
                ...
                insertStatement = f'''
                insert into WH_SALES_TARGETS1
                        ( TBL,BUSINESS_LINE_NAME,MONTH,YEAR,TARGET
                        )
                        values
                        (
                            ?,
